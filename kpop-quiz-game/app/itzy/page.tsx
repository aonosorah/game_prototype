...
import songsData from "../assets/itzy.json";
    ...
    const fetchSong = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/songs?track=${encodeURIComponent(trackName)}&artist=ITZY`
        );
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen p-4 bg-black text-white overflow-hidden">
      <div
        ...
        style={{
          backgroundImage: `url('https://billboard-com-br.s3.amazonaws.com/wp-content/uploads/2025/06/17103808/itzy.jpg')`,
          filter: "brightness(0.25) blur(1px)",
        }}
      />
      <Link
        href="/"
        className="fixed top-4 left-4 z-50 bg-pink-600/20 backdrop-blur-md p-2 rounded-lg border border-pink-500/50 text-pink-500 hover:bg-pink-500 hover:text-white transition-all"
      >
        ← Menu
      </Link>
      <div className="fixed top-4 right-4 flex flex-col gap-2 bg-slate-900/80 backdrop-blur-md p-4 rounded-xl border border-pink-500/30 shadow-2xl z-50">
      ...
      <div className="relative z-10 flex flex-col items-center mb-8">
        <p className="text-[10px] tracking-[0.4em] font-light text-pink-400 mt-2 ml-2 uppercase">
          ITZY
        </p>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center w-full">
        {!songInfo && (
          <p className="animate-pulse text-pink-300">
            Sincronizando com o ritmo de MIDZY...
          </p>
        )}
        ...
        {songInfo && !songInfo.error && (
          <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-500 w-full max-w-2xl">
            <div className="flex gap-2">
              <span className="bg-pink-500/20 text-pink-400 text-[10px] px-3 py-1 rounded-full border border-pink-500/30 uppercase font-bold tracking-tighter">
            ...
            <div className="relative w-56 h-56 md:w-72 md:h-72 mb-2">
              <img
                ...
                alt="Song Hint"
                className={`w-full h-full object-cover rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] border-2 border-white/10 transition-all duration-1000 ${
                  isRevealed
                    ? "blur-0 scale-100"
              ...
              />
              {!isRevealed && (
                <div className="absolute inset-0 flex items-center justify-center text-7xl font-bold text-pink-500/20">
                  ?
                </div>
              )}
            </div>
            ...
            <button
              ...
              className={`w-20 h-20 rounded-full border-4 flex items-center justify-center text-3xl transition-all ${
                isPlaying
                  ? "border-pink-500 scale-90 opacity-50 shadow-[0_0_25px_#ec4899]"
                  : "border-white hover:scale-110 hover:bg-pink-500/20"
              }`}
            >
              {isPlaying ? "🎶" : "▶️"}
            </button>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 w-full px-4 md:px-0 flex justify-items-center">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (choice === currentSong.track) {
                      ...
                      alert(`Oops! A música era: ${currentSong.track}`);
                      setScore(0);
                  ...
                  className={`p-4 rounded-xl border-2 transition-all w-full max-w-[260px] font-semibold backdrop-blur-sm ${
                    isRevealed && choice === currentSong.track
                      ...
                      : "bg-slate-900/60 border-slate-700 hover:border-pink-500 text-slate-200"
                  }`}
                >
                  {choice}
                </button>
